...
![社内ナレッジチャットの動作デモ。社内文書にある質問には出典付きで回答し、文書に無い質問には「記載がありません」と正直に答える様子](docs/images/rag-chat-demo.gif)

実際に動作している様子です。社内文書に**根拠がある質問**には出典(参照したファイル名)付きで回答し、**根拠が無い質問**には推測せず「記載がありません」と正直に答えます(ハルシネーション抑制)。
