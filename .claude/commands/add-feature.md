...
1. Grepツールを使用し、機能名に関連するキーワードでソースコード(`src/AiRagTemplate/`)を検索する。
  ```bash
  Grep('[機能に関連するキーワード]', 'src/AiRagTemplate/')
  ```
...
1. 以下のコマンドを順番に実行し、ビルドと全テストがパスすることを確認する(ICU 無のためインバリアント指定)。
  ```bash
  Bash('export DOTNET_ROOT="$HOME/.dotnet"; export PATH="$HOME/.dotnet:$PATH"; export DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1; dotnet build')
  Bash('export DOTNET_ROOT="$HOME/.dotnet"; export PATH="$HOME/.dotnet:$PATH"; export DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1; dotnet test')
  Bash('export DOTNET_ROOT="$HOME/.dotnet"; export PATH="$HOME/.dotnet:$PATH"; export DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1; dotnet format --verify-no-changes')
  ```
...
- ステップ7: `dotnet build`, `dotnet test`, `dotnet format --verify-no-changes` の全てがエラーなく成功する。
